...
onAuthStateChanged(auth, (user) => {
    if (user) {
        const uid = user.uid;
        const shoppingListInDB = ref(database, `users/${uid}/shoppingList`);
        ...
        onValue(shoppingListInDB, function (snapshot) {   //updates the database after any change
            if (snapshot.exists()) {
        ...
        function addToCart(item) {
            ...
            checkbox.addEventListener("change", function () {
                let exactLocationOfItemInDB = ref(database, `users/${uid}/shoppingList/${itemID}`);
